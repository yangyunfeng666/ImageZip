...
jstring Java_com_kye_imagezip_ImageZipUtil_compressBitmap1(JNIEnv* env,
...
Java_com_kye_imagezip_ImageZipUtil_compressBitmap(JNIEnv *env, jobject instance,
                                                  jobject bitmapcolor, jint w, jint h, jint quality,
                                                  jbyteArray fileNameStr_, jboolean optimize) {
    AndroidBitmapInfo infocolor;
    BYTE* pixelscolor;
    int ret;
    BYTE * data;
    BYTE *tmpdata;
    char * fileName = jstrinTostring(env, fileNameStr_);
    if ((ret = AndroidBitmap_getInfo(env, bitmapcolor, &infocolor)) < 0) {
        LOGE("AndroidBitmap_getInfo() failed ! error=%d", ret);
        return (*env)->NewStringUTF(env, "0");;
    }
